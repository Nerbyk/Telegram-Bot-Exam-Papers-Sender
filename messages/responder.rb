...
class MessageResponder
  attr_reader :bot, :message, :user_input, :my_text, :client_id, :verification, :import_info, :db
  ...
  def call(bot:, message:, user_input:, db:)
    ...
    @verification = Verification.new.call(client_id: client_id, db: db)
    respond(client_id)
  end
