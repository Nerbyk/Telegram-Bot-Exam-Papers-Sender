...
class MakeInlineMsg
  ...
  def get_markup
    kb = []
    markup = []
    for i in 0..@inline_items.length-1
      kb << Telegram::Bot::Types::InlineKeyboardButton.new(text:"#{@inline_items[i][0]}", callback_data: "#{@inline_items[i][1]}")
    end
    return markup = Telegram::Bot::Types::InlineKeyboardMarkup.new(inline_keyboard: kb)
  end
  ...
  def get_link
    ...
    markup = []
    for i in 0..@inline_items.length-1
      kb << Telegram::Bot::Types::InlineKeyboardButton.new(text:"#{@inline_items[i][0]}", url: "#{@inline_items[i][1]}")
    end
...
module UserInfo
  ...
  def self.get(bot, client_id)
    i = 0 # counter for steps
    import_arr = []
    name,vk_link,photo,status = String.new
    subjects = []
    puts("POSOSI LOSHARA")
    bot.listen do |message|
      case message.text
      when message.text
        ...
        when 2 # third step = get examination subjects
          ...
          subjects << message.text if !check_each_subject(message.text,bot)
  ...
  def self.check_name(name)
    ...
    return false if name.split(' ').length > 2 # if more, than 2 strings were passed
  ...
  def self.check_each_subject(message,bot)
    ...
    if available_list.include?(message)
      ...
      bot.api.send_message(chat_id: $client_id, text: "Пожалуйста используйте кливиатуру снизу для ввода предметов.") if !available_list.include?(message)
      return true
    end
  end
  def self.check_all_subjects(subjects,bot)
      ...
      #  for_admin(bot) if message.from.id == Config::ADMIN_ID
          case message
            ...
            when Telegram::Bot::Types::Message
              case message.text
              when '/start'
                start_msg(bot,message) if !(DataBase.new(id: $client_id).check_id)
                start_in_prog_msg(bot,message) if DataBase.new(id: $client_id).check_id == 'in progress'
                start_accepted_msg(bot,message) if DataBase.new(id: $client_id).check_id == 'accepted'
              else
